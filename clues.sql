...
SELECT name, population FROM countries WHERE population > 0 AND region = 'Southern Europe' ORDER BY population ASC LIMIT 1;

-- Clue #2: Now that we're here, we have insight that Carmen was seen attending language classes in this country's officially recognized language. Check our databases and find out what language is spoken in this country, so we can call in a translator to work with you.

-- Write SQL query here
SELECT countries.code AS country, countrylanguages.language AS language
FROM countries
JOIN countrylanguages ON countries.code = countrylanguages.countrycode
WHERE population > 0 AND region = 'Southern Europe' ORDER BY population ASC LIMIT 1;

-- Clue #3: We have new news on the classes Carmen attended – our gumshoes tell us she's moved on to a different country, a country where people speak only the language she was learning. Find out which nearby country speaks nothing but that language.

-- Write SQL query here

SELECT countries.code AS country, countries.name AS country_name
FROM countries
JOIN countrylanguages ON countries.code = countrylanguages.countrycode
...
AND MAX(CASE WHEN countrylanguages.language = 'Italian' THEN 1 ELSE 0 END) = 1
